...
- Null
"""
function find_reeb_nodes!(A::sGTDA,M,Ar;filter_cols=nothing,nbins_pyramid=2,overlap=(0.5,0.5),labels = [],smallest_component=50,component_size_thd=0,split_criteria="diff",split_thd=0.01,max_iters=50,verbose=false)
     ...
     mx = 0
     for (k,v) in A.final_components_unique
          temp  = StatsBase.countmap(labels[v])
          for (_,vvvv) in temp
               if vvvv > 1
                    mx += 1
               end
...
function filter_tiny_components!(A::sGTDA;node_size_thd=10,verbose=false,labels = [],smallest_component = 50)
     nodes = []
     ...
     println("Number of samples included before filtering:", length(nodes))
     println("Number of reebnodes before filtering tiny components out", length(A.final_components_unique))
     @show "node_size_thd" node_size_thd
     ...
     @info "After filtering tiny components " A.final_components_filtered length(nodes) sum([length(i) == 0 for i in A.node_assignments])
     mx = 0
     for (k,v) in A.final_components_unique
          temp  = StatsBase.countmap(labels[v])
          for (_,vvvv) in temp
...
function merge_reeb_nodes!(A::sGTDA,Ar,M;niters=1,node_size_thd=10,edges_dists=[],verbose = false,labels = [])
     ...
     @show sum([length(i) == 0 for i in A.node_assignments])
     ...
     for _ in range(1,niters)
          ...
          println("making the merging map")
          @time merging_map = sparse(merging_ei,merging_ej,ones(length(merging_ei)),num_components,num_components)
          @time merging_map = make_graph_symmetric(merging_map,num_components)
	     println("merging tiny nodes")
          @time merging_tiny_nodes!(A,merging_map,node_size_thd=node_size_thd,verbose=verbose)
     end
     
     println("updating A.node_assignments and A.node_assignments_tiny_components after merging tiny nodes and A.final_components_filtered")
     
     @time for (key,val) in A.final_components_filtered
          for v in val 
               append!(A.node_assignments[v],key)
          end
     end
     @show sum([length(i) == 0 for i in A.node_assignments])
     
     println("counting label mixing")
     mx = 0
     @time for (k,v) in A.final_components_filtered
          temp  = StatsBase.countmap(labels[v])
          for (_,vvvv) in temp
               if vvvv > 1
                    mx += 1
...
function merging_tiny_nodes!(A::sGTDA,merging_map;node_size_thd=10,verbose=false)
     ...
     #if verbose
          println("Number of samples included after merging:", length(nodes))
     #end
end
...
function build_reeb_graph!(obj::gnl,A::sGTDA,M;reeb_component_thd=1,max_iters=10,labels = [],is_merging=true,edges_dists=nothing,verbose=false)
     all_edge_index = [[], []]
     extra_edges = [[],[]]
     @info "Building reeb graph"
     
     @info "Updating node assignments"
     ...
     while (modified) && (is_merging) && (length(components_removed) > 0) && (curr_iter < max_iters)
          ...
          for cr in components_removed
	          ...
	          neighs = sparse(tmp_edges_dists').rowval
               ...
               #valid_neighs should also check whether that node has at all been included in the reeb graph this far
               valid_neighs = setdiff(valid_neighs,[i for i in range(1,size(M,1)) if length(A.node_assignments[i]) == 0])
	          tmp_edges_dists = tmp_edges_dists[:,valid_neighs]
     ...
     mx = 0
     for (k,v) in A.final_components_filtered
          temp  = StatsBase.countmap(labels[v])
          for (_,vvvv) in temp
               if vvvv > 1
                    mx += 1
     ...
     nodes = []
     for key in A.filtered_nodes
          append!(nodes,A.final_components_filtered[key])
     end
     nodes = sort(unique!(nodes))
     println("Number of samples included after building reeb graph:", length(nodes))
...
function gtdagraph!(A::gnl;overlap = 0.025,max_split_size = 100,labels = labels,min_group_size=5,min_component_group=5,alpha=0.5,nsteps_preprocess=5,extra_lens=nothing,is_merging=true,split_criteria="diff",split_thd=0,is_normalize=true,is_standardize=false,merge_thd=1.0,max_split_iters=200,max_merge_iters=10,nprocs=1,degree_normalize_preprocess=1,verbose=false)

     @time M,Ar = smooth_lenses!(A,extra_lens=extra_lens,alpha=alpha,nsteps = nsteps_preprocess,normalize = is_normalize,standardize = is_standardize,degree_normalize=degree_normalize_preprocess)

     @time edges_dists = select_merge_edges(A.G,M,merge_thd = merge_thd)

     this = time()
     gtda = sGTDA()
    
     @time find_reeb_nodes!(gtda,M,Ar,smallest_component=max_split_size,labels = labels,filter_cols=[i for i in range(1,size(M,2))],overlap=overlap,component_size_thd=0,split_criteria=split_criteria,split_thd=split_thd,max_iters=max_split_iters,verbose=verbose)
     
     if verbose
          @info "Initial number of reebnodes found " length(gtda.final_components_unique)
     end

     @time filter_tiny_components!(gtda,node_size_thd=min_group_size,smallest_component=max_split_size,labels = labels,)



     @time merge_reeb_nodes!(gtda,Ar,M,niters=max_merge_iters,node_size_thd=min_group_size,labels = labels,edges_dists=edges_dists,verbose = verbose)
     ...
     @time extra_edges = build_reeb_graph!(A,gtda,M,reeb_component_thd=min_component_group,labels = labels,max_iters=max_merge_iters,is_merging=is_merging,edges_dists=edges_dists,verbose=verbose)
