...
function firstexamplepart2()
    ...
    allothermethods = load_object("stored_embeddings/mnist.jld2")
    ncols = div(length(allothermethods),2)
    nrows = 2
    fig, axs = subplots(nrows, ncols, figsize=(40, 10))
...
function plot_fmnist()
    ...
    tight_layout()
    fig.savefig("draft2/fmnist.png")
    time_taken = Dict()
    push!(time_taken,"gtdr"=>gtdarecord["time"])

    allothermethods = load_object("stored_embeddings/fmnist.jld2")
    ncols = div(length(allothermethods),2)
    nrows = 2
    fig, axs = subplots(nrows, ncols, figsize=(40, 10))
...
function plot_coil20()
    name = "coil20"
    gtdarecord = JSON.parsefile("stored_embeddings/$(name).json")
    G_reeb = sparse(gtdarecord["G_reeb"][1],gtdarecord["G_reeb"][2],ones(length(gtdarecord["G_reeb"][2])))
    rc = gtdarecord["rc"]
    rn = gtdarecord["rn"]
    gtdatime = gtdarecord["time"]
    ...
    for (i, key) in enumerate(layout_keys)
        ...
        for i in range(1,size(xy,1))
            ...
            showlabels = [k for k in distkey if !(k in seen)]
            union!(seen, distkey)
            ax = draw_pie(dist_dict,xy[i,1],xy[i,2],sqrt(length(rc[i])/1e5),ax,color_map,showlabels,lw = 0.5)
        end
        ax.set_title("GTDR-$(match(r" (\w+)_layout",key).captures[1])")
        ax.axis("off")
        for lbl in labels_unique
        ax.scatter([], [], c=[color_map[lbl]], label=lbl)
        end
        ax.legend()
        println("$(key) done")
    end
    ...
    allothermethods = load_object("stored_embeddings/$(name).jld2")
    ncols = div(length(allothermethods),2)
    nrows = 2
    fig, axs = subplots(nrows, ncols, figsize=(40, 10))
...
function plot_melanoma()
    ...
    layout_keys = collect(keys(coords_dict))
    n,d = size(X)
    mnistlegend = ["unresolved","T","B","Macro","Endo","CAF","NK","Malignant"]
...
function plot_humandevelopmental()
    ...
    #for name in ["spectral","spring","kamada_kawai"]
    X = hcat(gtdarecord["orig_data"]...)
    labels = gtdarecord["orig_labels"]["actual_labels"]
    ...
    layout_keys = collect(keys(coords_dict))
    n,d = size(X)
    mnistlegend = unique(gtdarecord["orig_labels"]["true_labels"])
...
function plot_synthdev()
    ...
    labels_unique = unique(labels)
    cmap = PyPlot.cm.get_cmap("tab10")  # 20 colors
    color_map = Dict(lbl => cmap(mod1(i, 10) - 1) for (i, lbl) in enumerate(labels_unique))
    nodecolors = [color_map[lbl] for lbl in labels]
    labelcounter = zeros(length(labels_unique))
